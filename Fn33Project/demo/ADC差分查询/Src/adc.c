...
static uint8_t GetDifferentialChannelSample(uint32_t group, uint32_t *ADCRdresult)
{
    uint32_t counter = 0;
    uint32_t EOC_Flag;
    uint8_t State;
    ...
    FL_ADC_DisableSequencerChannel(ADC, FL_ADC_ALL_CHANNEL);        //清空打开的通道
    FL_ADC_EnableSequencerChannel(ADC, group);                      //通道选择
    FL_ADC_EnableDifferentialChannel(ADC, group);                   //配置差分使能
    FL_ADC_ClearFlag_EndOfConversion(ADC);                          //清标志
    FL_ADC_Enable(ADC);                                             //启动ADC
    FL_ADC_EnableSWConversion(ADC);                                 //开始转换
    do
    ...
    if(EOC_Flag == 0x01U)
    {
        ...
        *ADCRdresult = FL_ADC_ReadConversionData(ADC);                   //获取采样值
        ...
        FL_ADC_Disable(ADC);                                            //关闭ADC
        FL_ADC_DisableSequencerChannel(ADC, group);                     //通道关闭
        FL_ADC_DisableDifferentialChannel(ADC, group);                  //配置差分禁止
        State = 0;
    }
    else
    {
...
int32_t GetDifferentialChannelVoltage_POLL(uint32_t group)
{
    uint32_t Get122VSample = 0, GetVSample = 0;
    int32_t  GetChannelVoltage = 0;
    uint8_t  Get122VSample_State, GetVSample_State;

    Get122VSample_State = GetVREF1P2Sample_POLL(&Get122VSample);
    GetVSample_State = GetDifferentialChannelSample(group, &GetVSample);


    if((Get122VSample != 0) && (Get122VSample_State == 0) && (GetVSample_State == 0))
    {
        GetChannelVoltage = (int32_t)((((int64_t)GetVSample - 2048) * 3000 * (ADC_VREF)) / ((int64_t)Get122VSample * 4095) * 2);
    }
    // 转换结果
