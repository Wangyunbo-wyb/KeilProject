...
void COMPx_IRQHandler(void)
{
    ...
    if(FL_SET == FL_COMP_IsActiveFlag_Comparator1(COMP))
    {
    ...
    //COMP2
    //注意对输入信号的滤波，否则容易误触发中断
    if(FL_SET == FL_COMP_IsActiveFlag_Comparator2(COMP))
    {
    ...
    //COMP3
    //注意对输入信号的滤波，否则容易误触发中断
    if(FL_SET == FL_COMP_IsActiveFlag_Comparator3(COMP))
    {
        FL_COMP_ClearFlag_Comparator3(COMP);                    //清除COMP中断标志

        if(FL_COMP_OUTPUT_HIGH == FL_COMP_GetOutput(COMP3))     //比较器, P 电压高于 N
        {
            //用户代码
        }
        else                                                    //比较器3, P 电压低于 N
...
//比较器3 查询方式
void COMP3_Query(void)
{
    //比较器3 查询
    if(FL_COMP_GetOutput(COMP3) == FL_COMP_OUTPUT_HIGH)         //P 电压高于 N
    {
        //用户代码
    }
    else//P 电压低于 N
