...
{
    FL_DelayUs(93 * Num); //1 etu=93 us(4M),或者1 etu=186 us(2M)
}
...
uint8_t Judge_Edge(void)                                 //判定沿
{
    ...
    {
        if(0x01UL == FL_U7816_IsActiveFlag_RXBusy(U7816)) //1=处于数据接收状态，收到起始位置1，收到停止位清零
        {
            return 1;    //下降沿
        }
...
uint8_t Get_7816_Char(unsigned char *Data)
{
    ...
    for(i = 0x00; i < 10000; i++) //约900ms(4M),或者1800ms(2M)
    {
        ...
        if(0x01UL == FL_U7816_IsActiveFlag_RXComplete(U7816)) //RX_FLAG已经置位
        {
            *Data = FL_U7816_ReadRXBuff(U7816);
            return 1; //成功
        }

        if(0x01UL == FL_U7816_IsActiveFlag_Error(U7816)) //ERROR_FLAG已经置位
        {
...
uint8_t Snd_7816_Byte(unsigned char Data)
{
    ...
    for(i = 0; i < 400; i++) //约39ms(4M),或者78ms(2M)
    {
        Delay_Netu(1);

        if(0x01UL == FL_U7816_IsActiveFlag_TXBuffEmpty(U7816)) //TX_FLAG置位
        {
            return 0; //数据发送成功
        }

        if(0x01UL == FL_U7816_IsActiveFlag_Error(U7816))  //ERROR_FLAG置位
        {
...
uint8_t ColdReset(void)
{
    ...
    FL_DelayMs(11);
    //-------- T1时刻, RST输出高电平 --------
    FL_GPIO_SetOutputPin(GPIOC, FL_GPIO_PIN_10); //RST输出高电平
    FL_U7816_EnableRX(U7816); //卡复位完成后才能启动接收,之前IO电平不稳定
    (void)FL_U7816_ReadRXBuff(U7816);  //RX启动后, 会产生rx_flag=1, 必须通过读取RXBUF来清除此标志.
    FL_U7816_ClearFlag_TXParityError(U7816); //清TxParityError标志，使U7816ERR中相应的错误标志清零
    FL_U7816_ClearFlag_RXParityError(U7816);
    FL_U7816_ClearFlag_RXFrameError(U7816);
    FL_U7816_ClearFlag_RXOverflow(U7816);
...
uint8_t U7816Transfer(void)
{
    ...
    for(i = 0; i < 5; i++)
    {
        (void)Snd_7816_Byte(sent_data[i]);
    }
